...
export default function AppContent(props) {
    ...
    const checkForCorrectRoute = (isAsync = false) => {
        if (router.pathname !== `/login`) {
            if (!isAsync) {
                if (router.pathname === "/[...pages]") return router.replace("/");
                else return Promise.resolve(saveLastVisitedPage());
            }
            if (!userState.isAuthenticated) return router.replace(`/login`);
    ...
    return (
        <main className={`flex flex-row min-h-max w-full relative`}>
            {isLoading && (
                <>
                    <div
                        className={`z-40 bg-black opacity-80 fixed w-full h-full top-0 left-0`}
                    />
                    <Spinner
                        classes={`z-50 fixed top-1/2 left-1/2`}
                        size={90}
                        removeMargin
                    />
                </>
            ...
            {router.isFallback || userState.isAsync ? (
                ...
                <div
                    className={`max-w-screen w-screen min-h-screen overflow-hidden m-0 `}
                >
                    {props.children}
                </div>
            )}
