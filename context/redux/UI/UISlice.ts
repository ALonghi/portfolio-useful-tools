...
export const UISlice = createSlice({
    ...
    reducers: {
        addNotificationEvent: (
            state,
            {payload: newEvent}: PayloadAction<ToastData>
        ) => {
            state.events.push(newEvent);
        },
        hideNotificationById: (
            state,
            {payload: eventId}: PayloadAction<string>
        ) => {
            state.events = state.events.map((e) =>
                e.id === eventId ? {...e, hidden: true} : e
            );
        },
...
export const isLoadingSelector = (state: RootState) =>
    state.uiReducer?.isLoading;
export const lastEventSelector = (state: RootState) =>
    state.uiReducer?.events[state.toastReducer?.events?.length - 1];
