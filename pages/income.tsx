...
const IndexPage = () => {
  ...
  const [focusedMovementId, setFocusedMovementId] = useState<string | null>(
      null
  );
  ...
  useEffect(() => {
    if (userState.isAuthenticated && !movementsState.isAsync) fetchMovements();
  }, [userState.isAuthenticated]);

  const toggleInactiveStatus = async (movement: IMovement) => {
    try {
      const partialUpdate = await MovementService.updateMovementStatus(
          movement.id,
          !movement.isInactive
      );
      dispatch(
          updateMovements(
              movementsState.movements.map((m) =>
                  m.id === movement.id ? {...m, ...partialUpdate} : m
              )
          )
      );
      ...
      const errorMsg = `Error while updating movement status: ${
          e.message || e
      }`;
        ...
        {movs.map((m) => (
            ...
            >
              ...
              >
                ...
                <div
                    className={`ml-auto flex flex-row flex-1 flex-nowrap justify-end items-center min-w-fit mr-2`}
                >
                  <p className={`inline-flex  ml-4 text-gray-700`}>
                    {Utils.formatAmount(m.amount)} CHF
                  </p>
                  <p
                      className={`inline-flex  ml-3 text-white ${
                          m.reason === "necessary" ? "bg-blue-500" : "bg-yellow-400"
                      } font-light
              ...
              <div
                  onClick={() => toggleInactiveStatus(m)}
                  className={`w-min sm:ml-8`}
              >
                {m.isInactive ? (
                    <EyeSlashIcon
                        className={`cursor-pointer block opacity-40`}
                        height={20}
                    />
                ) : (
                    <EyeIcon
              ...
              <XMarkIcon
                  className={`cursor-pointer block opacity-40 ml-4`}
                  height={20}
      ...
      <>
        <div
          ...
          {showPopup && (
              <MovementPopup
                  showPopup={showPopup}
                  toggleRequestSent={() => fetchMovements()}
                  hidePopup={() => hidePopup()}
                  movement={
                      movementsState?.movements?.find(
                          (m) => m.id === focusedMovementId
                      ) || null
                  }
              />
          )}
          <div className={`w-full mx-auto`}>
            <main className="sm:flex">
              <div className="max-w-sm">
                ...
                <p className={` bottom-10 mt-4 text-gray-400`}>
              ...
              <div className="sm:ml-6 w-full">
                <div className="sm:border-l sm:border-gray-200 sm:pl-6">
                  <div
                      className={`flex align-middle items-center justify-around`}
                  >
                    <div className={`flex items-center`}>
                      ...
                      <p
                          className={`ml-3 font-light text-base text-gray-700 w-max`}
                      >
                        (net monthly)
                      </p>
                    </div>
                  ...
                  {movementList(
                      movementsState.movements
                          ...
                          ?.sort(
                              (a: IMovement, b: IMovement) =>
                                  -1 * a.reason.localeCompare(b.reason)
                          )
                  )}
                </div>
                <div className="sm:border-l sm:border-gray-200 sm:pl-6">
