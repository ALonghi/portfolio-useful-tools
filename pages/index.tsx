...
const IndexPage = () => {
  ...
  const [calculated, setCalculated] = useState<CalculatedResult>(
      {} as CalculatedResult
  );
            ...
            <main className="sm:flex">
              <div className="max-w-xs">
                <p className="text-4xl font-bold text-indigo-600 sm:text-5xl">
                  Calculate your investment compound gains
                </p>
              ...
              <div className="sm:ml-6 w-max">
                <div className="sm:border-l sm:border-gray-200 sm:pl-6">
                  <div className="block mt-2">
                    <InputForm
                        name={`startCapital`}
                        label={`Initial capital to start with`}
                        type={`number`}
                        placeholder={`5.000`}
                        value={startCapital}
                        updateValue={setStartCapital}
                    />
                  </div>

                  <h1 className="my-4 text-2xl font-bold text-gray-900 tracking-tight sm:text-3xl">
                    Amount to invest
                  </h1>
                  ...
                  <div className={`mt-5`}>
                    ...
                    <InputFormWithTail
                        ...
                        tailText={`%`}
                        value={interestRate}
                        updateValue={(e) => setInterestRate(e.target.valueAsNumber)}
                    />
                  </div>
                  <div className={`mt-5`}>
                    <InputForm
                        name={`years`}
                        label={`Years to keep the money invested`}
                        type={`number`}
                        placeholder={`10`}
                        inputClasses={`max-w-[6rem]`}
                        value={years}
                        updateValue={setYears}
                    />
                  </div>
                  <button
                      type="button"
