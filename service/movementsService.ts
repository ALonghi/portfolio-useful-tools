...
export default class MovementService {
    ...
    static addMovement = async (newMovement: Partial<IMovement>, userId: string): Promise<void> => {
        try {
            ...
            const message = `Problem occurred when adding movement: ${e.message || e} (${JSON.stringify(newMovement)})`
    ...
    static deleteMovement = async (movementId: string): Promise<void> => {
        try {
            ...
            const message = `Problem occurred when deleting movement: ${e.message || e} (${movementId})`
            console.error(message)
            return Promise.reject(message)
        }
    }

    static updateMovementStatus = async (movementId: string, isInactive: boolean): Promise<Partial<IMovement>> => {
        try {
            ...
            const message = `Problem occurred when updating movement status: ${e.message || e} (${movementId},${isInactive})`
            console.error(message)
            return Promise.reject(message)
        }
    }

    static updateMovement = async (movement: Partial<IMovement>): Promise<Partial<IMovement>> => {
        try {
            ...
            const message = `Problem occurred when updating movement ${e.message || e} (${JSON.stringify(movement.id)})`
            console.error(message)
            return Promise.reject(message)
        }
    }
